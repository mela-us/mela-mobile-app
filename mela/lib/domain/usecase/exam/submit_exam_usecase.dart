...
class SubmitExamUsecase extends UseCase<ExerciseResult, SubmitResultParams> {
  // final UpdateProgressApi _updateProgressApi;
  final SaveResultApi _saveResultApi;
  final RefreshAccessTokenUsecase _refreshAccessTokenUsecase;
  final LogoutUseCase _logoutUseCase;

  SubmitExamUsecase(this._saveResultApi, this._refreshAccessTokenUsecase,
      this._logoutUseCase);
  @override
  Future<ExerciseResult> call({required SubmitResultParams params}) async {
    try {
      ExerciseResult result =
          await _saveResultApi.saveResult(params, '/api/test-histories');
      print("Update Test history: ${result.message}");
      return result;
    } catch (e) {
      print("Exception in Test history: ${e.toString()}");
