...
class UploadImageExamUsecase
    extends UseCase<List<List<String>?>, List<List<File>>> {
  final ExamRepository _examRepository;
  final RefreshAccessTokenUsecase _refreshAccessTokenUsecase;
  final LogoutUseCase _logoutUseCase;

  UploadImageExamUsecase(this._examRepository, this._refreshAccessTokenUsecase,
      this._logoutUseCase);

  @override
  Future<List<List<String>?>> call({required List<List<File>> params}) async {
    try {
      return await _examRepository.uploadImages(params);
    } catch (e) {
