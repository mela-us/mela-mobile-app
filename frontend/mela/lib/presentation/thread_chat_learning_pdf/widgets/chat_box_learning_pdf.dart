...
class ChatBoxLearningPdf extends StatefulWidget {
  const ChatBoxLearningPdf({super.key});

  @override
  _ChatBoxLearningPdfState createState() => _ChatBoxLearningPdfState();
}

class _ChatBoxLearningPdfState extends State<ChatBoxLearningPdf> {
  final TextEditingController _controller = TextEditingController();
  // late final _controllerMath = MathFieldEditingController();
  // bool _isMathMode = false;
  final _chatBoxLearningPdfStore = getIt.get<ChatBoxLearningPdfStore>();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _chatBoxLearningPdfStore.setShowSendIcon(false);
      _chatBoxLearningPdfStore.clearImages();
    });
    ...
    disposerIsDisplayCamera = reaction<Conversation>(
      (_) => _threadChatLearningPdfStore.currentConversation,
      (value) {
        ...
        //   _chatBoxLearningPdfStore.setShowCameraIcon(false);
  ...
  void onTextChanged() {
    _chatBoxLearningPdfStore.setText(_controller.text);
    if ((_controller.text.isNotEmpty ||
            _chatBoxLearningPdfStore.images.isNotEmpty) &&
        _threadChatLearningPdfStore.isLoading == false) {
      _chatBoxLearningPdfStore.setShowSendIcon(true);
    } else {
      _chatBoxLearningPdfStore.setShowSendIcon(false);
    }
  }
  ...
  //   _chatBoxLearningPdfStore.removeImage(image);
  //   if (_chatBoxLearningPdfStore.images.isEmpty && _controller.text.isEmpty) {
  //     _chatBoxLearningPdfStore.setShowSendIcon(false);
  //   }
  //   // WidgetsBinding.instance.addPostFrameCallback((_) {});
  // }
  ...
  void _showImagePickerOptions() {
    if (_threadChatLearningPdfStore.isLoading) {
      return;
    }
          ...
          children: [
            ListTile(
              ...
              onTap: () {
                ...
                _chatBoxLearningPdfStore.pickImage(ImageSource.camera);
              },
            ),
            ListTile(
              ...
              onTap: () {
                Navigator.pop(context);
                _chatBoxLearningPdfStore.pickImage(ImageSource.gallery);
              },
            ),
          ],
        );
  ...
  Widget _buildListImagesIsSeclected() {
    return Observer(builder: (_) {
      return Padding(
        ...
        child: _chatBoxLearningPdfStore.images.isEmpty
            ? const SizedBox()
            : SingleChildScrollView(
                ...
                child: Row(
                  ...
                  children: [
                    ..._chatBoxLearningPdfStore.images.map((image) {
                      return Padding(
  ...
  Widget _buildTextField() {
    return Observer(builder: (_) {
      // print(
      //     "==================> _buildTextField: showSendIcon ${_chatBoxLearningPdfStore.showSendIcon} showCameraIcon ${_chatBoxLearningPdfStore.showCameraIcon}");
      return Row(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          _chatBoxLearningPdfStore.showCameraIcon
              ? GestureDetector(
            ...
            child: Container(
                ...
                child: kIsWeb || Platform.isAndroid
                    ...
                    : CupertinoTextField(
                        ...
                        placeholder:
                            "Đặt câu hỏi cho Mela về bài giảng này nhé",
          ...
          _chatBoxLearningPdfStore.showSendIcon
              ? GestureDetector(
                  onTap: () async {
                    if (_threadChatLearningPdfStore.tokenChat == 0) {
                    ...
                    List<File> images =
                        _chatBoxLearningPdfStore.images.toList();
                    _chatBoxLearningPdfStore.clearImages();
                    await _threadChatLearningPdfStore.sendChatMessage(
                        message, images);
